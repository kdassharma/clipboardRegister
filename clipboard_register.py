...
if CURRENT_CLIPBOARD_CONTENT != TEMPORARY_CLIPBOARD_CONTENT:
    ...
    message = "{}: {}".format(current_time, TEMPORARY_CLIPBOARD_CONTENT)
    label = tk.Label(tempFrame, text=message)
...
#         message = "{}: {}".format(current_time, TEMPORARY_CLIPBOARD_CONTENT)
#         label = tk.Label(tempFrame, text=message)
#         label.place(relx=0.5,rely=0.5,relwidth=0.5, relheight = 0.5, anchor = 'n')
